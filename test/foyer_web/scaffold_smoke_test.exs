defmodule FoyerWeb.ScaffoldSmokeTest do
  ...
  describe "Profile / Me" do
    # F.Profile.1, F.Profile.11 — identity header + points balance
    ...
    # F.Profile.1 — identity header fields via element IDs
    test "Me renders stats row with recognitions-this-month element", ctx do
      conn = sign_in(ctx.conn, ctx.maya)
      {:ok, view, _html} = live(conn, ~p"/me")
    ...
    # F.Profile.18 — off-shift redirect
    test "F.Profile.18 — /me redirects off-shift user to /today", ctx do
      conn = sign_in(ctx.conn, ctx.jamal)
      assert {:error, {:redirect, %{to: "/today"}}} = live(conn, ~p"/me")
    end

    # F.Profile.25 — property code from config
    test "F.Profile.25 — renders property code in identity header", ctx do
      conn = sign_in(ctx.conn, ctx.maya)
      {:ok, view, _html} = live(conn, ~p"/me")
  ...
  describe "People directory" do
    ...
    # F.Profile.19 — colleague profile reuses profile_card
    ...
    # F.Profile.6, F.Profile.8 — private recognitions hidden, given absent
    test "F.Profile.6, F.Profile.8 — colleague view hides private recognitions and given section",
         ctx do
      # Add a private recognition for Maya from Charlotte
      alias Foyer.Recognitions.Recognition

      {:ok, _private} =
        %Recognition{}
        |> Recognition.changeset(%{
          sender_id: ctx.charlotte.id,
          recipient_id: ctx.maya.id,
          body: "Private note about Maya for testing F.Profile.6",
          values: ["care"],
          bonus_points: 0,
          public: false
        })
      ...
      # Maya views Charlotte's profile — Given section absent, no private recogs
      conn = sign_in(ctx.conn, ctx.maya)
      {:ok, view, _html} = live(conn, ~p"/people/#{ctx.charlotte.id}")

      html = render(view)
      # Given section must be absent
      refute has_element?(view, "#recognitions-given"),
             "Given section must be absent on colleague view"

      # Private recognition must not appear
      refute html =~ "Private note about Maya for testing F.Profile.6",
             "Private recognition must not appear on colleague's profile view"
    end
  end
end
